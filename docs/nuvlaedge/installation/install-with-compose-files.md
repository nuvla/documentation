...
2. from the [edge panel](https://nuvla.io/ui/edge), add a new `nuvlaedge`, and either
...
- **Upgrade/Downgrade the entire NuvlaEdge Engine installation**: let's say we want to upgrade your existing NuvlaEdge Engine installation to the latest release in [GitHub](https://github.com/nuvlaedge/deployment/releases). Then:
    ...
    4. resume the NuvlaEdge Engine installation by running `docker-compose -p nuvlaedge -f docker-compose.yml -f docker-compose.usb.yml -f docker-compose.network.yml up -d`
